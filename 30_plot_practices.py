...
sample_practices = practices[np.random.choice(len(practices), size=sample_size, replace=False)]
...
for practice in tqdm(sample_practices, desc="Generating plots"):
    ...
    for kind, color, marker in (("drought","red","o"),("flood","blue","o"),("flood_geo","purple","x")):
        if kind not in practice_data:
            continue
        flagged_dates = practice_data["date"].where(practice_data[kind]==1, drop=True).values
        flagged_values = practice_data["items"].sel(date=flagged_dates).values
        plt.scatter(flagged_dates, flagged_values, c=color, s=60, marker=marker, label=f"{kind.title()} Flag", zorder=5)
